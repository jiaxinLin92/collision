...
#define TEST_SURFACE
    ...
    // my plane use four points
//    auto floor = new MYPlane<float>(GMlib::Point<float,3>(-10,10,-1),
//                                GMlib::Point<float,3>(10,10,-1),
//                                GMlib::Point<float,3>(10,-10,-1),
//                                GMlib::Point<float,3>(-10,-10,-1));
...
//    _scene->insert(con);
    ...
    GMlib::Vector<float,3> vec000(-10,-10,-1);
    GMlib::Vector<float,3> vec001(0,-10,-1);
    GMlib::Vector<float,3> vec002(10,-10,-1);

    GMlib::Vector<float,3> vec010(-10,0,-1);
    GMlib::Vector<float,3> vec011(0,0,-15);
    GMlib::Vector<float,3> vec012(10,0,-1);

    GMlib::Vector<float,3> vec020(-10,10,-1);
    GMlib::Vector<float,3> vec021(0,10,-1);
    GMlib::Vector<float,3> vec022(10,10,-1);

    m0[0][0] = vec000; m0[0][1] = vec001; m0[0][2] = vec002;
    m0[1][0] = vec010; m0[1][1] = vec011; m0[1][2] = vec012;
    m0[2][0] = vec020; m0[2][1] = vec021; m0[2][2] = vec022;

    auto nine =new MYPlanenine<float>(m0);


    nine->toggleDefaultVisualizer();
    nine->getDefaultVisualizer()->setDisplayMode(GMlib::Visualizer::DISPLAY_MODE_WIREFRAME);
    ...
    auto left = new MP(GMlib::Point<float,3>(-10,10,-1),//define a begin points
                       GMlib::Vector<float,3>(0,0,3),//vector z  direction
    ...
    auto right = new MP(GMlib::Point<float,3>(10,10,-1),//define a begin points
                           GMlib::Vector<float,3>(0,-20,0),//vector y direction
                           GMlib::Vector<float,3>(0,0,3));//vector z  direction
    right->toggleDefaultVisualizer();
    right->replot(50,50,1,1);
    con->addWall(right);
    //_wallArray+=left;

    // mp:wall(behind)
    auto behind = new MP(GMlib::Point<float,3>(-10,10,-1),//define a begin points
                           GMlib::Vector<float,3>(20,0,0),//vector x direction
                           GMlib::Vector<float,3>(0,0,3));//vector z  direction
    behind->toggleDefaultVisualizer();
    behind->replot(50,50,1,1);
    con->addWall(behind);

    // mp:wall(front)
    auto front = new MP(GMlib::Point<float,3>(-10,-10,-1),//define a begin points
 ...
 //    First sphere(Testsphere)
    auto sphere1 = new Testsphere(1,GMlib::Vector<float,3>( 3.0f, 3.0f, 0.0f),1,nine);
    con->addSphere((sphere1));
    sphere1->enableDefaultVisualizer();
    sphere1->translate(GMlib::Vector<float,3>( -2.0f, -2.0f, 0.0f));
    sphere1->setMaterial(GMlib::GMmaterial::Gold);
    sphere1->replot(20,20,1,1);


    //sphere2
    auto sphere2 = new Testsphere(1,GMlib::Vector<float,3>( 0.0f, -10.0f, 0.0f),1,nine);
    con->addSphere((sphere2));
    sphere2->enableDefaultVisualizer();;
    sphere2->setMaterial(GMlib::GMmaterial::Jade);
    sphere2->translate(GMlib::Vector<float,3>( 0.0f, 2.0f, 0.0f));
    sphere2->replot(20,20,1,1);



     //sphere3
    auto sphere3 = new Testsphere(1,GMlib::Vector<float,3>( 10.0f, -5.0f, 0.0f),1,nine);
    con->addSphere((sphere3));
    sphere3->enableDefaultVisualizer();
    sphere3->setMaterial(GMlib::GMmaterial::Pearl);
